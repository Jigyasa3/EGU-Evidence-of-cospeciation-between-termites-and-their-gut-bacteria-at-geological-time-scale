...
sister1 <- args[2] #p__Fibrobacterota
sister2 <- args[3]
output2 <- args[4] #no.of termitespecific clades >=10 species
suffix1 <- args[5] #suffix for termitespecific clades tips files
...
#remove the sistergroups + gtdb ids
cogtree<-read.newick(file=tree)
d<-data.frame(label=cogtree$tip.label)
d$phyla<-gsub("^.*_d__","d__",d$label)

sistergroup1<-as.vector(as.character(sister1)) #user defined
sistergroup2<-as.vector(as.character(sister2)) #user defined

sistergroup<-paste(sistergroup1,sistergroup2,sep = '|') #for str_detect
...
#dnew<-d%>%filter(groupinfo=="phylaofinterest")
...
sistergroupfind_list<-mapply(sistergroupfind,nodenumber_list,as.vector(as.character(sistergroup1)))
...
nodenumber_list<-list() #initialize a list
nodenumber_list<-as.list(gtdbtermite_df2_final$V1) #add values to empty list
sistergroupfind_list<-mapply(sistergroupfind,nodenumber_list,as.vector(as.character(sistergroup2)))
